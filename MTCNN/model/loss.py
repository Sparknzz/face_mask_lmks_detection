...
class LossFn:
    ...
    def cls_loss(self, gt_label, conf_logits):
        conf_logits = torch.squeeze(conf_logits) # 512, 
        gt_label = torch.squeeze(gt_label) # 512, 

        # get the mask element which >= 0, only 0 and 1 and 2 can effect the detection loss
        mask = torch.ge(gt_label, 0)
        valid_gt_label = torch.masked_select(gt_label, mask)
        valid_conf_logits = torch.masked_select(conf_logits, mask)

        return self.loss_cls(valid_conf_logits, valid_gt_label)*self.cls_factor
    ...
    def box_loss(self,gt_label,gt_offset,pred_offset):
        ...
        unmask = torch.eq(gt_label,0) # find out all 0 label they are 1, all 1 and -1 label value is 0.
