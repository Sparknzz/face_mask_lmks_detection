...
def metric_hit(probability, truth, threshold=0.5):
    batch_size, num_class, H, W = probability.shape # 512, 1, 1, 1

    with torch.no_grad():
        probability = probability.view(batch_size, -1) # 512, 1
        truth = truth.view(batch_size, -1) # 512, 1
        mask = torch.ge(truth, 0)

        valid_truth = torch.masked_select(truth, mask)
        valid_probability = torch.masked_select(probability, mask)

        p = (valid_probability > threshold).float()
        t = valid_truth.float()
        ...
        num_neg = valid_truth.size()[0] - num_pos
    ...
    for cur_epoch in range(1, end_epoch+1):
        ...
        for batch_idx, (image, (gt_label, gt_bbox, gt_landmark)) in enumerate(train_data):
            ...
            if batch_idx%frequent==0:
                tn, tp, num_neg, num_pos = metric_hit(torch.sigmoid(conf_logits), gt_label)
        ...
        print("Epoch: %d, cls loss: %s, bbox loss: %s" % (cur_epoch, cls_loss_avg, bbox_loss_avg))
...
if __name__ == "__main__":
    train_pnet(annotation_file='/root/face_mask_lmks_detection/MTCNN/annos/pnet_train_imglist.txt', model_store_path='/root/face_mask_lmks_detection/MTCNN/weights')
